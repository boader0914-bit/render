...
noMatch(html, /tokenInput|operatorToken|관리자 토큰/u);
...
match(html, /id="openAdCapture"[^>]*disabled/u);
...
match(html, /<script src="\/naver-ad-snapshot-handoff-contract\.js" defer><\/script>/u);
...
match(css, /grid-template-columns:\s*170px minmax\(260px, 1fr\) 126px/u);
...
match(js, /createCaptureSearchUrl/u);
match(js, /createCaptureNonce/u);
match(js, /validateHandoffMessage/u);
match(js, /globalThis\.open\(captureUrl/u);
match(js, /globalThis\.addEventListener\("message"/u);
match(js, /event\.source !== pending\.popup/u);
match(js, /event\.origin !== "https:\/\/search\.naver\.com"/u);
match(js, /crypto\?\.randomUUID/u);
noMatch(js, /x-v2-basic-operator-token|tokenInput|elements\.token/u);
match(js, /serviceStatus\?\.authRequired === true/u);
...
equal((js.match(/https?:\/\//gu) || []).length, 1);
