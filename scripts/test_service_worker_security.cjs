...
async function main() {
  ...
  const currentCache = "lodging-datalab-pwa-v20260802-ui-release-v29";
  ...
  const staticResponse = await dispatchFetch({
    ...
    mode: "cors",
    destination: "script",
    url: "https://fixture.local/app.js?v=v2-20260802-ui-release-v29"
  });
  assert.equal(await staticResponse.text(), "fresh:https://fixture.local/app.js?v=v2-20260802-ui-release-v29");
  await Promise.resolve();
  await Promise.resolve();
  assert.ok(putCalls.some((call) => call.key.includes("/app.js?v=v2-20260802-ui-release-v29")), "allowlisted static asset is cached");
