...
> 상태: 차단됨. `N3-D1-Commit` 이후 별도 승인된 `N3-Copy-Only-Live`가 성공하고 `N3-Commit`이 완료되기 전에는 이 초안을 실행하지 않는다. Phase 4에서도 원형+복제본 연속 live pair를 다시 사용하지 않고, 기능별 단독 canary 승인 방식을 적용해야 한다.

당신은 데이터랩 재구축 4단계를 담당하는 시니어 수집 시스템 개발자다.

목적은 Phase 3 live와 commit으로 확인된 네이버 예약업체 ID를 입력으로, V2의 예약 상품
