...
관리자 화면은 `설정 → API·연동 → 관광지 방문 전망`에서 사용한다. 시도와 시군구를 고른 뒤 **관광지 불러오기**를 누르고, 반환된 목록에서 정확한 관광지를 선택한다. 이름 검색과 관광지 선택은 받은 목록만 사용하므로 외부 API를 추가 호출하지 않는다. 날짜별 목록은 접거나 펼칠 수 있다.
