...
function companyRankInsight(item = {}, fallbackRank = 0) {
  ...
  if (hasInventory && Number.isFinite(rate)) {
    ...
    } else if (rate <= B2B_LOW_RESERVATION_RATE && remaining > 0) {
      tone = "watch";
...
function b2bCompanyActionProfile(item = {}, insight = companyRankInsight(item)) {
  ...
  if (!linked) {
  ...
  } else if (Number.isFinite(rate) && rate <= B2B_LOW_RESERVATION_RATE && remaining > 0) {
    tone = "watch";
    label = B2B_LOW_RESERVATION_LABEL;
    ...
    tone = "strong";
    label = "경쟁 여지";
    summary = "잔여 객실과 노출 흐름을 함께 비교할 수 있는 업체입니다.";
  }
  return {
    tone,
    label,
...
function b2bRankBoardModel(items = rankedCompanyItems()) {
  const rows = items.slice(0, 30).map((item, index) => {
    ...
    const opportunityScore = (linked ? 30 : 8)
      ...
      + (Number.isFinite(rate) && rate <= B2B_LOW_RESERVATION_RATE && remaining > 0 ? 22 : 0)
      + (remaining >= 3 ? 8 : 0)
  ...
  const gapRows = linkedRows.filter((row) => Number.isFinite(row.rate) && row.rate <= B2B_LOW_RESERVATION_RATE && row.remaining > 0);
  ...
  const decision = rankOnlyRows.length > linkedRows.length
    ...
    : gapRows.length >= 2
      ? { label: "잔여 경쟁권", tone: "strong", summary: "상위 노출 경쟁업체 중 잔여 객실이 있는 구간을 가격, 상품 구성, 채널 상태와 함께 봅니다." }
      : hotRows.length >= 2
...
function b2bRankRangeModel(model = b2bRankBoardModel()) {
  ...
  return buckets.map((bucket) => {
    ...
    const gapRows = bucket.rows.filter((row) => row.linked && Number.isFinite(row.rate) && row.rate <= B2B_LOW_RESERVATION_RATE && row.remaining > 0);
...
function b2bRankExposureModel(model = b2bRankBoardModel()) {
  ...
  const gapRows = model.gapRows || linkedRows.filter((row) => Number.isFinite(row.rate) && row.rate <= B2B_LOW_RESERVATION_RATE && row.remaining > 0);
  ...
  const revenueTotal = revenueRows.reduce((sum, row) => sum + finiteNumber(row.insight?.revenue, 0), 0);
  ...
  const cards = [
    {
      ...
      note: `상세 표본 ${fmtNumber(topRows.filter((row) => row.linked).length)}개`,
      tone: topRows.some((row) => Number.isFinite(row.rate) && row.rate >= B2B_HIGH_RESERVATION_RATE) ? "hot" : "strong"
    },
    {
      label: "상세 표본 확보",
      value: Number.isFinite(linkedCoverage) ? fmtRate(linkedCoverage) : "대기",
      note: `${fmtNumber(linkedRows.length)}/${fmtNumber(rows.length)}개 예약 수량 연결`,
      tone: Number.isFinite(linkedCoverage) && linkedCoverage >= 0.55 ? "good" : "watch"
    },
    {
      label: B2B_LOW_RESERVATION_LABEL,
      value: fmtNumber(gapRows.length),
      note: gapRows.length ? `잔여 ${fmtNumber(remainingRooms)}실 확인` : "해당 업체 없음",
    ...
    },
    {
      ...
      note: revenueRows.length ? `합산 ${fmtWon(revenueTotal)}` : "가격 표본 필요",
  ...
  const actionRows = [
    ...
    },
    {
      ...
      summary: "네이버 노출은 있으나 예약 수량 연결이 필요한 업체입니다.",
  ...
  const focusRows = (model.focusRows || rows.slice(0, 4)).map((row) => {
    ...
    return {
      ...
      note: row.linked
        ? `잔여 ${fmtNumber(row.remaining)}실 · ${insight.revenue ? fmtWon(insight.revenue) : "매출 표본 대기"}`
        : "상세 예약 수량 연결 필요"
    };
  });
  return {
    ...
    summary: Number.isFinite(linkedCoverage)
      ? `상세 표본 ${fmtRate(linkedCoverage)} 기준으로 상위 노출과 ${B2B_LOW_RESERVATION_LABEL} 업체를 같이 봅니다.`
  ...
  return `
    <section class="b2b-rank-brief ${escapeHtml(model.decision.tone)}">
      ...
      <div class="b2b-rank-metrics">
        ...
        <article><span>${B2B_LOW_RESERVATION_LABEL}</span><strong>${fmtNumber(model.gapRows.length)}</strong><small>잔여 객실 보유</small></article>
        <article><span>${B2B_HIGH_RESERVATION_LABEL}</span><strong>${fmtNumber(model.hotRows.length)}</strong><small>예약 수량 표본 기준</small></article>
      </div>
...
function b2bDetailPositionModel(item = {}) {
  ...
  if (!insight.hasInventory) {
    ...
    tone = "hot";
    label = B2B_HIGH_RESERVATION_LABEL;
    summary = "상위 노출과 수집기간 예약율 40% 이상이 함께 확인됩니다. 가격, 객실 구성, 주말 잔여 재고를 벤치마크합니다.";
  ...
  if (!isAdminRole()) {
    ...
    els.summaryGrid.innerHTML = `
      ...
      <article class="summary-card public-summary-card">
        ...
        <div><strong>${fmtNumber(brief.remainingSupply)}</strong><small>경쟁권 잔여 객실 추정</small></div>
      </article>
      <article class="summary-card public-summary-card">
...
function b2bMarketDecision(score, rate, opportunityCount) {
  if (score >= 78 && opportunityCount >= 3) {
    return {
      label: "경쟁 기회",
      tone: "strong",
      summary: "검색 수요와 경쟁업체 잔여 객실 표본이 함께 보여 매출, 가격, 상품 구성을 비교하기 좋은 지역입니다."
    };
  }
  if (score >= 64) {
    return {
      label: "경쟁 비교",
      tone: "watch",
      summary: "상위 노출 경쟁업체의 판매율, 잔여 객실, 채널 표본을 중심으로 시장 위치를 비교합니다."
    };
  }
  if (Number.isFinite(rate) && rate >= 0.6) {
    return {
...
function b2bMarketBriefModel(data = state.data || {}) {
  ...
  const opportunityRows = items
    ...
    .slice(0, 5);
  const lowSalesCount = opportunityRows.filter((row) => row.lodging.rate < 0.35).length;
  ...
  const actionRows = [
    ...
    },
    {
      ...
      detail: gapCount
        ...
        : "상위 노출 업체의 예약율과 잔여 객실을 기준값으로 봅니다.",
      tab: "rank",
      button: "순위 보기"
    },
    {
...
function b2bSimpleSummaryModel(
  ...
  const revenueSampleText = revenueReady
    ...
    : "매출 표본 대기";
  const confidenceText = Number.isFinite(priceCoverage)
    ? fmtRate(priceCoverage)
    : (Number.isFinite(brief.revenueCoverage) ? fmtRate(brief.revenueCoverage) : "확인 대기");
  const trendValue = trend.reason
    ? "추세 대기"
    : trend.hasSeries
      ? demandTrendLabel()
      : "데이터랩 대기";
  const trendNote = trend.reason
    ? trend.reason
    : trend.hasSeries && Number.isFinite(trendStats.recentChange)
      ? `최근 3개월 ${formatSignedRate(trendStats.recentChange)}`
      : "12개월 추세 표본 기준";
  const hotCount = finiteNumber(rankModel.hotRows?.length, 0);
  const gapCount = finiteNumber(rankModel.gapRows?.length, 0);
  ...
  const cards = [
    {
      ...
      note: revenueSampleText
    },
    {
      tone: hotCount ? "hot" : gapCount ? "strong" : "neutral",
      label: "경쟁 강도",
      value: hotCount ? `${B2B_HIGH_RESERVATION_LABEL} ${fmtNumber(hotCount)}곳` : `${fmtNumber(rankModel.rows.length || brief.itemCount)}곳 비교`,
      note: gapCount ? `${B2B_LOW_RESERVATION_LABEL} 업체 ${fmtNumber(gapCount)}곳` : "상위 노출 업체 기준"
    },
    {
    ...
    },
    {
      ...
      note: Number.isFinite(detailCoverage)
        ? `상세 연결 ${fmtRate(detailCoverage)}`
        : "가격·예약 연결 기준"
    }
  ];
  const actions = [
...
function b2bPublicOverviewModel(brief = b2bMarketBriefModel()) {
  ...
  const actionRows = [
    ...
    },
    {
      ...
      label: "노출 순위",
      value: `${fmtNumber(rankModel.rows.length)}개`,
      detail: `네이버 상위 노출 경쟁업체와 예약 표본 ${fmtNumber(rankModel.linkedRows.length)}개를 함께 봅니다.`,
      tab: "rank",
      button: "순위 보기"
    },
    {
          ...
          <div class="b2b-brief-grid">
            <article class="b2b-insight-panel">
              <div class="report-card-head">
                <div>
                  <h3>경쟁 해석</h3>
                  <p>경쟁업체의 판매율, 잔여 객실, 상품 구성을 고객 관점으로 요약합니다.</p>
                </div>
              </div>
...
function trendMonthNumber(label = "") {
  const match = String(label || "").match(/(\d{1,2})월/);
  if (!match) return NaN;
...
function b2bDemandOutlookModel(traffic = demandTrafficAggregate(), playbook = b2bDemandPlaybookModel(traffic)) {
  ...
  const cards = [
    ...
    },
    {
      ...
      note: market.averageRevenue ? `평균 ${fmtWon(market.averageRevenue)} · 상위 ${fmtWon(snapshot.topRevenueTotal)}` : "가격·수량 표본 필요"
    }
  ];
  ...
  const actionRows = [
    {
      ...
      detail: peakNow || rising
        ...
        : "보합/방어권에서는 노출 순위와 상세 표본 확보 범위를 우선 확인합니다."
    },
    {
      label: "가격/재고",
    ...
    },
    {
      ...
      detail: rankModel.hotRows.length
        ...
        : "상위 노출 업체의 예약 표본을 확보해 향후 컨택 판단 근거로 씁니다."
    }
  ];
  const competitorRows = rankModel.rows.slice(0, 5).map((row) => {
    const revenue = finiteNumber(row.insight?.revenue, 0);
    return {
      itemIndex: row.linked ? finiteNumber(row.item.availabilityIndex, -1) : -1,
      rank: row.rank,
      name: row.item.name || "업체명 확인",
      tone: row.insight?.tone || "neutral",
...
function renderB2BDemandOutlook(traffic = demandTrafficAggregate(), playbook = b2bDemandPlaybookModel(traffic)) {
  ...
  return `
    <div class="b2b-demand-outlook">
      ...
      `}
      <div class="b2b-demand-outlook-layout">
        ...
        <article class="b2b-demand-competitors">
          <div>
            <strong>수요 대응 경쟁업체</strong>
            <small>상위 노출 업체의 판매율과 매출 표본</small>
          </div>
          ${model.competitorRows.length ? model.competitorRows.map((row) => {
            const attrs = row.itemIndex >= 0 ? `type="button" data-open-company="${row.itemIndex}"` : `type="button" disabled`;
            return `
              <button class="${escapeHtml(row.tone)}" ${attrs}>
                <span>${row.rank ? `${fmtNumber(row.rank)}위` : "순위"}</span>
                <strong>${escapeHtml(row.name)}</strong>
                <em>${escapeHtml(row.rate)}</em>
                <small>${escapeHtml(`${row.revenue} · ${row.note}`)}</small>
              </button>
            `;
          }).join("") : `<p>경쟁업체 표본이 없습니다.</p>`}
        </article>
      </div>
    </div>
  `;
