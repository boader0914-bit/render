...
미관측·부분수집·오류는 0명으로 바꾸지 않습니다. 직전 12개월과 전년동월은 새로
수집하지 않으므로 기존 전년 비교형 방문자 보조점수는 적용하지 않습니다. 별도의
12개월 내부 추세 모델이 확정되기 전에는 가중치를 0%로 두고 기존 지표만 다시 정규화합니다.

관광 수요 강도는 `AreaTarDemDsService`의 월별 `관광 체류 강도`와
`관광 소비 강도`를 서로 분리해 저장·표시합니다. 일반 화면은 저장된 Snapshot만
읽고, 관리자는 선택 지역의 최근 완료월 기준 12개월만 별도로 갱신할 수 있습니다.
공개 API 호출에는 화면과 그래프에 사용하는 전체 지표코드인 체류 `21`, 소비 `22`를
각각 명시합니다. 선택 지표코드를 생략한 요청에서 발생할 수 있는 `03 NODATA_ERROR`를
실제 지역자료 부재로 오인하지 않으며, 정상 응답을 받은 값만 Snapshot으로 저장합니다.
Persistent Disk 운영환경에서는
산청군을 먼저 적재한 뒤 전국 시군구의 최근 완료월 기준 12개월을
채우는 백그라운드 작업도 실행합니다. 개발계정 일일 1,000회 한도에 여유를
...
기록되므로 배포나 재시작 후에도 이어집니다. 최초 12개월 적재가 끝나면 최신 완료월만
